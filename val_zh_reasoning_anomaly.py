...
#     # args = parse_args()
#     input_json='/data/cj/RS_StarSight/src/datasets/valid/data/sub_json/anomaly_detection_and_interpretation.json'
#     image_root='/data/cj/valid_contest/valid_images'
#     model_dir='/data/cj/RS_StarSight/tool/FM9G4B-V'
#     output_json='/data/cj/RS_StarSight/src/datasets/valid/test/eval_anomaly.json'
#     run_eval(input_json, image_root, model_dir, output_json)
# ========== CLI（最小改动） ==========
def parse_args():
    ap = argparse.ArgumentParser(description="FM9G MCQ evaluator")
    ap.add_argument("--input_json",  required=True, help="题目 JSON 路径")
    ap.add_argument("--image_root",  required=True, help="图像根目录")
    ap.add_argument("--model_dir",   required=True, help="模型目录（如 FM9G4B-V）")
    ap.add_argument("--output_json", required=True, help="结果输出 JSON 路径")
    return ap.parse_args()
